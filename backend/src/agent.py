...
# -------------------------
# Simple Product Catalog (Amlan Dey's Shop)
# -------------------------
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Madu', the friendly shopkeeper and voice assistant for Amlan Dey's Shop.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT (Amlan Dey's Shop) — Madu")
    ...
    # Start the agent session with the GameMasterAgent (Madu)
    await session.start(
